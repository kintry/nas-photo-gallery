#!/usr/bin/env python3
"""
NAS相册 Web 服务 v2 — NAS本地运行版
跑在 OECT NAS (Armbian) 上，直接读取本地照片
访问地址: http://192.168.2.104:5000

关键区别 vs WSL版：
- 去掉所有SFTP/SSH操作，全部改为本地文件读取
- 缩略图生成直接在本地处理，无multiprocessing超时（本地极快）
- `_get_raw_cached` 直接读本地文件
"""
...
def get_thumbnail(local_path, size=200):
    """生成缩略图（缓存到本地）"""
    ...
    try:
        ...
        quality = 65 if size <= 200 else 75
        out = io.BytesIO()
...
def batch_precache_thumbnails(albums, max_per_album=999999):
    ...
    for album in albums:
        ...
        for p in photos_cache[:max_per_album]:
            ...
            if fn.startswith('.') or fn.startswith('._'):
                continue
            if p.get('size', 0) > 10_000_000:  # 超过10MB跳过（NAS本地，放宽限制）
                continue
            ck = hashlib.md5(p['path'].encode()).hexdigest()
            sm = THUMB_DIR_SMALL / f"{ck}.jpg"
            lg = THUMB_DIR_LARGE / f"{ck}.jpg"
            if not sm.exists() or not lg.exists():
