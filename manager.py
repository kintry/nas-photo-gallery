...
async function scanPhotoRoots(onlyNew) {
  // 扫描设备上未配置的新相册：平铺列出（每个目录=一个相册，照片数含其下所有子目录），供全选/全不选/单选
  ...
  try {
    // 启动后台扫描（立即返回），然后轮询 scan_progress 获得进度和结果
    await apiPR(prHost, 'scan');
    let data = null;
    for (var _k = 0; _k < 180; _k++) {
      var _p;
      try { _p = await apiPR(prHost, 'scan_progress'); } catch(_e) { _p = null; }
      if (_p && typeof _p.progress === 'number') {
        var _pct = _p.progress || 0;
        var _cur = _p.current ? (' · ' + _p.current.slice(0,50)) : '';
        area.innerHTML = '<div style="color:#7f8c8d;font-size:13px;padding:10px">📡 正在扫描相册目录... <b style="color:#e8e8e8">' + _pct + '%</b>' + _cur + '</div>';
        if (!_p.running && (_pct >= 100 || (_p.result && _p.result.length))) { data = _p; break; }
      }
      await new Promise(function(r){ setTimeout(r, 800); });
    }
    var all = (data && data.result) || [];
    // 只留未配置的
    ...
    const esc = escHtml;
    const items = pending.map(function(d, i){
      return '<div style="display:flex;align-items:center;padding:8px 10px;border:1px solid #24334f;border-radius:8px;margin-bottom:5px;background:#16213e">' +
        '<input type="checkbox" id="prc_' + i + '" data-idx="' + i + '" onchange="updatePrChecked()" style="width:auto;margin:0 10px 0 0;flex-shrink:0">' +
        '<div style="flex:1;font-size:12px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="' + esc(d.path) + '">' +
          esc(d.path) +
          (d.photo_count ? ' <span style="color:#95a5a6;font-size:11px">(' + d.photo_count + '张)</span>' : '') +
        '</div>' +
      '</div>';
    }).join('');
    ...
    area.innerHTML =
      '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">' +
        '<div style="font-size:12px;color:#7f8c8d">🗂 发现 <b style="color:#e8e8e8">' + pending.length + '</b> 个未配置相册目录（每个含其下所有子目录照片）</div>' +
      '</div>' +
      '<div style="display:flex;gap:8px;margin-bottom:8px">' +
        '<button class="btn btn-secondary" onclick="prCheckAll(true)" style="flex:1;padding:8px;font-size:13px">☑️ 全选</button>' +
        '<button class="btn btn-secondary" onclick="prCheckAll(false)" style="flex:1;padding:8px;font-size:13px">⬜ 全不选</button>' +
      '</div>' +
      '<div style="max-height:280px;overflow-y:auto">' + items + '</div>' +
      '<button class="btn btn-success" onclick="addNewPhotoRootsAll()" style="width:100%;margin-top:10px;padding:13px;font-size:15px;border-radius:10px">✅ 添加选中（<span id="prAddCount2">0</span> 个）</button>' +
    ...
    // 默认全选（方便直接添加）
    var boxes = document.querySelectorAll('#prScanArea input[type=checkbox][data-idx]');
...
// 全选 / 全不选
function prCheckAll(sel) {
  var boxes = document.querySelectorAll('#prScanArea input[type=checkbox][data-idx]');
  boxes.forEach(function(b){ b.checked = !!sel; });
  updatePrChecked2();
}

// 更新"添加选中"计数
function updatePrChecked2() {
  const boxes = document.querySelectorAll('#prScanArea input[type=checkbox][data-idx]');
  let n = 0;
  boxes.forEach(function(b){ if (b.checked) n++; });
  const el = document.getElementById('prAddCount2');
  if (el) el.textContent = n;
}

// 单选勾选时同步计数（旧 updatePrChecked 保留兼容）
function updatePrChecked() { updatePrChecked2(); }

// 递归渲染子目录树（保留但不再用于新增界面，兼容旧调用）
function scanRenderRecursive(dirList, level) {
  const ind = level * 16;
  return (dirList||[]).map(function(d){
    return '<div style="display:flex;align-items:center;padding:5px 6px;padding-left:' + (10+ind) + 'px;border-radius:6px;margin-bottom:2px">' +
      '<span style="flex:1;font-size:12px;color:#c8c8c8" title="' + escHtml(d.path) + '">📁 ' + escHtml(d.name) + '</span>' +
    '</div>';
  }).join('');
}

// 全部添加勾选的未配置目录（改为按勾选添加，不再全量）
async function addNewPhotoRootsAll() {
  const area = document.getElementById('prScanArea');
  const all = window.__prPendingAdd || [];
  const boxes = document.querySelectorAll('#prScanArea input[type=checkbox][data-idx]');
  const paths = [];
  boxes.forEach(function(b, i){
    if (b.checked && all[i]) paths.push(all[i].path);
  });
  ...
  try {
    ...
    // 添加成功后：不关闭弹窗，显示明确成功反馈 + 提供"完成"按钮（避免突然刷新导致用户误以为没成功）
    var addedCount = (data.added ? data.added.length : paths.length);
    var albumsNow = (data.albums != null ? data.albums : '?');
    area.innerHTML =
      '<div style="color:#2ecc71;padding:14px;text-align:center;border:1px solid #2ecc71;border-radius:8px;font-size:14px">' +
        '✅ 添加成功！共 ' + addedCount + ' 个相册目录<br>' +
        '<span style="font-size:12px;color:#95a5a6">当前相册总数：' + albumsNow + ' 个</span><br><br>' +
        '<button class="btn btn-success" onclick="closePhotoRootsModal()" style="padding:9px 26px;font-size:14px">✅ 完成</button>' +
      '</div>';
    showPRMsg('ok', '添加成功！共 ' + addedCount + ' 个');
    window.__prPendingAdd = [];
    // 延迟刷新已配置列表（让用户先看到成功反馈，不打断扫描弹窗）
    setTimeout(function(){ loadPhotoRoots(prHost); }, 300);
  } catch(e) {
...
def create_app():
    ...
    @app.route('/api/device/<host>/photo_roots/scan_progress')

    def api_device_photo_roots_scan_progress(host):

        return jsonify(_device_photo_roots_request(host, 'scan_progress'))
