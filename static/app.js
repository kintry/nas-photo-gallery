...
(function() {
  ...
  function renderAlbums(albums) {
    ...
    albumGrid.innerHTML = albums.map(a => {
      ...
      return `<div class="album-card" onclick="openAlbum('${escJsStr(a.path)}')">
  ...
  // 转义 JS 字符串字面量（用于 onclick 内嵌路径）：反斜杠 \\ 防止被当转义符吞掉；单引号转义
  function escJsStr(s) { return escHtml(s).replace(/\\/g,'\\\\').replace(/'/g,"\\'"); }
