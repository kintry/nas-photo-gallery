...
(function() {
  ...
  // ── 状态 ──
  let currentAlbum = '';          // 当前相册路径
  let allPhotos = [];             // 当前相册的所有照片
  let currentPage = 1;
  let totalPages = 1;
  let currentIndex = -1;          // 大图浏览中当前照片索引
    ...
    albumGrid.innerHTML = albums.map(a => {
      ...
      return `<div class="album-card" onclick="openAlbum('${escHtml(a.path)}')">
        ...
        <div class="album-info">
          ...
          <div class="album-count">${a.photo_count} 张照片</div>
        </div>
      </div>`;
    }).join('');
  }
  ...
  function renderPhotos(photos) {
    if (currentPage === 2) photoGrid.innerHTML = ''; // 第一页已显示loading, 第二页开始追加
    const html = photos.map((p, i) => {
      const idx = allPhotos.length - photos.length + i;
      return `<div class="photo-item" onclick="openViewer(${idx})">
  ...
  window.closeViewer = function(e) {
    if (e && e.target !== viewerOverlay && !e.target.closest('.btn-close')) return;
    viewerOverlay.style.display = 'none';
    viewerImage.src = '';
  ...
  function loadThumbStrip() {
    thumbTrack.innerHTML = allPhotos.map((p, i) =>
      `<img src="${thumbUrl(p)}" class="${i === currentIndex ? 'active' : ''}" onclick="openViewer(${i})" loading="lazy">`
    ).join('');
  }
